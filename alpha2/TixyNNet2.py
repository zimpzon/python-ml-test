...
class TixyNet2(nn.Module):
    def __init__(self, game, args):
        ...
        super(TixyNet2, self).__init__()

        self.fc1 = nn.Linear(self.board_x * self.board_y * 8, 1024)
    ...
    def one_hot_encode(self, boards):
        ...
        one_hot_planes = torch.zeros((batch_count, total_piece_types, boards.size(1), boards.size(2)), device=boards.device, dtype=torch.float32)

        # one_hot_planes should now be batch_count * 8 * board_x * board_y
        for batch_idx in range(batch_count):
            for i in range(1, 5): # these are the piece types. It sets a 1 in the correct plane for each piece type, for each batch.
                one_hot_planes[batch_idx, i-1] = (boards[batch_idx] == i) # planes 0-3
                one_hot_planes[batch_idx, i+3] = (boards[batch_idx] == -i) # planes 4-7

        return one_hot_planes.reshape(boards.size(0), -1)
